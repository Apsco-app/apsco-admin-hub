...
const ApplicantList: React.FC<ApplicantListProps> = ({ onViewDetails }) => {
  ...
  useEffect(() => {
    // Wait for the profile to load and ensure the user is a school owner
    if (authLoading || !profile || !profile.school_id) {
  ...
  if (!profile || !profile.school_id) return <p className="p-4">You must be logged in as a school owner to view applications.</p>;
